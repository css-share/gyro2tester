...
// pointer to the configuration registers needed to set hardware into loopback mode
...
int main(void)
{
	...
	//========= FPGA Test Sequence ==============================
	//
	// Write 48k data words(96k bytes) to the Tx Buffer in DMA
	// After data buffer has been filled TxLED(LED_0) should go on

	// Do this after Tx Buffer data has been written

	*(baseaddr_tx_fifo+0) = 0x00000001;
	xil_printf("Tx Fifo is enabled\r\n");

	// read back TxFifo levels
	xil_printf("TxFifo_REG3: 0x%08x\n\r", *(baseaddr_tx_fifo+3));

	*(baseaddr_channel+2) = 0x00000001;
	xil_printf("MCK in BiDirChannels is enabled\r\n");

	*(baseaddr_channel+0) = 0x01070000;
	xil_printf("Loopback mode in BiDirChannels is set\r\n");

	*(baseaddr_channel+1) = 0x00000011;
	xil_printf("Out and IN channels started in BiDirChannels\r\n");

	// data should now show up on TXD pin
	// RxLED(LED_1) should light up after Rx buffer is filled

	xil_printf("Fill level in RxFifo_REG3: 0x%08x\n\r", *(baseaddr_rx_fifo+3));

	*(baseaddr_rx_fifo+0) = 0x00000001;
	xil_printf("RxFifo read enabled\r\n");

	// data can now be read out of the RxFifo
	//
	//

	*(baseaddr_rx_fifo+0) = 0x00000000;
	xil_printf("RxFifo read disabled\r\n");

	// enable RxFifo to collect another 48k of data
	*(baseaddr_rx_fifo+1) = 0x00000001;
	xil_printf("RxFifo read disabled\r\n");

	// data can now be read again
	//
	//

	//
	//==== End of FPGA Test Sequence ============================
	//===========================================================
