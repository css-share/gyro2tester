...
#ifdef PRINT_DEBUGS
    xil_printf("\r\n\nTesting Rx captures in non-bypassed mode\n\r");
...
#ifdef PRINT_DEBUGS
    xil_printf("Tx Fifo Levels after enabled%x \r\n", XAxi_ReadReg(TXFIFO_REG3));
#endif
...
#ifdef PRINT_DEBUGS
	 xil_printf("Tx Fifo Levels after fill%x \r\n", XAxi_ReadReg(TXFIFO_REG3));
#endif

// below is only needed if using Rx bypass mode for RXD straight into DDR
/*
#ifdef PRINT_DEBUGS
...
#ifdef PRINT_DEBUGS
	 xil_printf("Enable RX FIFO PUSH & POP  \r\n");
#endif
...
#ifdef PRINT_DEBUGS
    unsigned int numValuesToPrint = 4;

    print("Values in Rx DMA TS buffer: \n\r");
	for(Index = 0; Index < numValuesToPrint; Index++) {
		xil_printf("%04x\r\n",(unsigned int)RxBufferPtr[Index]);
	}

    print("Values in Rx DMA Node buffer: \n\r");
	for(Index = 0; Index < numValuesToPrint; Index++) {
		xil_printf("%04x\r\n",(unsigned int)RxBufferPtr[Index+0x4000]);
	}

    print("Values in Rx DMA Anti-node buffer: \n\r");
	for(Index = 0; Index < numValuesToPrint; Index++) {
		xil_printf("%04x\r\n",(unsigned int)RxBufferPtr[Index+0x8000]);
	}
#endif
