...
// Instantiation of CPU Axi Bus Interface S00_AXI
  ...
  always_comb begin
   ...
   begin
     bram0_odd_data_a  = fifo_wdata[15:0];
     bram0_odd_addr_a  = write_pointer[12:0];    
     bram1_odd_data_a  = fifo_wdata[15:0];
     bram1_odd_addr_a  = write_pointer[12:0];
     bram2_odd_data_a  = fifo_wdata[15:0];
...
/*
    ...
    always @(posedge clk) begin // was tclk
     if (slv_reg0[16] && !wr_ptr_pattern[0] && (wr_ptr_pattern <= 16'hBFFF))
       mem_even[wr_ptr_pattern[15:1]] <= wr_ptr_pattern; 
     else if (fifo_wren & !txfifo_full)  
       mem_even[write_pointer] <= fifo_wdata[15:0]; 
    end 

 
    always @(posedge clk) begin // was tclk
     if (slv_reg0[16] && wr_ptr_pattern[0] && (wr_ptr_pattern <= 16'hBFFF))
       mem_odd[wr_ptr_pattern[15:1]] <= wr_ptr_pattern; 
     else if (fifo_wren & !txfifo_full) 
       mem_odd[write_pointer] <= fifo_wdata[31:16]; 
    end 
*/
...
/*
  ...
  always @(posedge clk or negedge rstn) begin  
    ...
    always @(posedge clk or negedge rstn) begin  
        ...
        begin
          if (!rd_ptr_reg[0])
            m00_data_reg <= mem_even[rd_ptr_reg[RD_ADDR_WIDTH-1:1]];
          else
            m00_data_reg <= mem_odd[rd_ptr_reg[RD_ADDR_WIDTH-1:1]];
        end
    end
    ...
    always @(posedge clk or negedge rstn) begin  
        ...
        begin  
          if  (!rd_ptr_reg[0])  
            m01_data_reg <= mem_even[rd_ptr_reg1[RD_ADDR_WIDTH-1:1]];
          else
            m01_data_reg <= mem_odd[rd_ptr_reg1[RD_ADDR_WIDTH-1:1]];
        end    
    end
    ...
    always @(posedge clk or negedge rstn) begin  
        ...
        begin
           if  (!rd_ptr_reg[0]) 
             m02_data_reg <=  mem_even[rd_ptr_reg2[RD_ADDR_WIDTH-1:1]];
           else
             m02_data_reg <=  mem_odd[rd_ptr_reg2[RD_ADDR_WIDTH-1:1]]; 
        end
    end  
