...
//    (* ram_style = "block" *)logic [15:0] mem2_even[(2**MEM_ADDR_WIDTH)-1:0];   // 8k 16bit word entries  
 ...
 /* 
  always @(posedge clk or negedge rst_n) begin 
    if (~rst_n)
      wren_odd <= 0;
    else if (reg_word0[16] && (wr_ptr_pattern <= 16'h3FFF) && wr_ptr_pattern[0])
      wren_odd <= 1'b1;
    else if (push &  wr_ptr_reg0[0])
      wren_odd <= 1'b1; 
    else
  ...
  always @(posedge clk) begin
    ...
    begin 
      if (reg_word0[16])   
       ...
       mem1_even[wr_ptr_reg0[MEM_ADDR_WIDTH:1]]     <= s00_axis_tdata[31:16];  
    end
  end
  ...
  always @(posedge clk) begin
    ...
    begin 
      if (reg_word0[16])   
       ...
       mem1_odd[wr_ptr_reg0[MEM_ADDR_WIDTH:1]]     <= s00_axis_tdata[31:16];  
    end
  end
  ...
  always @(posedge clk) begin
    ...
    begin 
      if (reg_word0[16])   
       ...
       mem2_even[wr_ptr_reg0[MEM_ADDR_WIDTH:1]]     <= s00_axis_tdata[15:0];  
    end
  end
  ...
  always @(posedge clk) begin
    ...
    begin 
      if (reg_word0[16])   
       ...
       mem2_odd[wr_ptr_reg0[MEM_ADDR_WIDTH:1]]     <= s00_axis_tdata[15:0];  
    end
  end
 ...
 */
  ...
  assign pop             = ok_to_pop & rx_mid_ok_to_push;
 ...
 always @(posedge clk or negedge rst_n) begin 
    ...
    begin  
      ...
      rdata_vld2 <= pop && (rd_ptr_reg < 16'h8000) && (rd_ptr_reg >= 16'h4000);   
 ...
 always_comb
   begin
     if (rd_ptr_reg < 16'h2000)
     begin
       rdata_upper = mem0_odd[rd_ptr_reg[12:0]];
       rdata_lower = mem0_even[rd_ptr_reg[12:0]];
     end 
     else if (rd_ptr_reg < 16'h4000)
     begin
       rdata_upper = mem1_odd[rd_ptr_reg[12:0]];
       rdata_lower = mem1_even[rd_ptr_reg[12:0]];
     end
     else   
     begin
       rdata_upper = mem2_odd[rd_ptr_reg[12:0]];
       rdata_lower = mem2_even[rd_ptr_reg[12:0]];
     end
   end


  assign m00_axis_tdata      = {rdata_upper, rdata_lower};
