...
// Instantiation of Axi Bus Interface S00_AXI
  ...
  assign enable    = DATA_WORD_0[4];

  assign spi_adr   = DATA_WORD_1[6:0];
  assign spi_wdata = DATA_WORD_2[15:0];
//  assign spi_busy = (cur_state != IDLE);

//  assign DATA_WORD_3 = {16'h0000, spi_rdata};
  assign DATA_WORD_3[15:0]  = {16'h0000, spi_rdata};
  assign DATA_WORD_3[31]    = spi_busy;
  assign DATA_WORD_3[30:16] = 15'h0000;            
  ...
  logic spi_busy_remove;
  logic spi_busy_remove_s;
  logic spi_busy_remove_ss;
  logic spi_busy_remove_clr; 
  
  always @(posedge spi_clk or negedge rstn) 
    begin  
    if (~rstn)
      spi_busy_remove <= 1'b0;
    else if ((cur_state == SHIFT) && (nxt_state == IDLE))
      spi_busy_remove <= 1'b1;
    else if ((cur_state == READ) && (nxt_state == IDLE))
      spi_busy_remove <= 1'b1;
    else
      spi_busy_remove <= 1'b0;
    end
  ...
  always @(posedge clk or negedge rstn) 
    ...
    begin  
       spi_busy_remove_s <= 0;
       spi_busy_remove_ss <= 0;
    end   
    else
    begin
       spi_busy_remove_s <= spi_busy_remove;
       spi_busy_remove_ss <= spi_busy_remove_s;      
    end 
    end

  assign spi_busy_clr = !spi_busy_remove_s & spi_busy_remove_ss;
  ...
  always @(posedge clk or negedge rstn) 
    ...
    if (~rstn) 
      spi_busy <= 0;
    else if (spi_busy_clr)
      spi_busy <= 0;
    else if (DATA_WORD_0[0])
      spi_busy <= 1'b1;
    end
 ...
 always @(*)
  ...
  always @(posedge spi_clk or negedge rstn) 
   ...
   assign spi_oe = !(cur_state == READ) & enable; // control the tristate for read
  ...
  assign spi_clk_valid = ((cur_state == SHIFT) || (cur_state == READ)); 
   ...
   assign SPI_SCK = ~spi_clk & spi_clk_valid & enable;
   assign SPI_CS = ~spi_valid & enable;
