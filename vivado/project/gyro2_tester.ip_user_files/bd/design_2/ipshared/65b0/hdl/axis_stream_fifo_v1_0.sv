...
// Instantiation of Axi Bus Interface S00_AXI
    ...
    reg [15:0] mem_alt0[(2**MEM_ADDR_WIDTH)-1:0];    // 32k 16bit word entries  
    reg [15:0] mem0[(2**MEM_ADDR_WIDTH)-1:0];        // 32k 16bit word entries  
    reg [15:0] mem_alt1[(2**MEM_ADDR_WIDTH)-1:0];    // 32k 16bit word entries  
    reg [15:0] mem1[(2**MEM_ADDR_WIDTH)-1:0];        // 32k 16bit word entries   
    reg [15:0] mem_alt2[(2**MEM_ADDR_WIDTH)-1:0];    // 32k 16bit word entries  
    reg [15:0] mem2[(2**MEM_ADDR_WIDTH)-1:0];        // 32k 16bit word entries  
//   
 ...
 // assign wr_ptr_reg2 = wr_ptr_reg0 + 16'h8000;
  ...
  assign empty = (wr_ptr_reg0 == 16'h0000);

  assign reg_word3[15:0]  = wr_ptr_reg0;
  ...
  always @(posedge clk) begin  
    ...
    begin
      if (!wr_ptr_reg0[0])
        mem0[wr_ptr_reg0[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[47:32];
      else
        mem_alt0[wr_ptr_reg0[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[47:32];
    end
  end
  ...
  always @(posedge clk) begin  
    ...
    begin
      if (!wr_ptr_reg0[0])
        mem1[wr_ptr_reg1[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[31:16];
      else
        mem_alt1[wr_ptr_reg1[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[31:16];
    end
  end
  ...
  always @(posedge clk) begin  
    ...
    begin
      if (!wr_ptr_reg0[0])
        mem2[wr_ptr_reg2[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[15:0];
      else
        mem_alt2[wr_ptr_reg2[WR_ADDR_WIDTH-1:1]] <= s00_axis_tdata[15:0];
    end
  end
 ...
 always_comb
   begin
     if (read_pointer < 4000)
     begin
       rdata_upper = mem_alt0[read_pointer[13:0]];
       rdata_lower = mem0[read_pointer[13:0]];
     end 
     else if (read_pointer < 8000)
     begin
       rdata_upper = mem_alt1[read_pointer[13:0]];
       rdata_lower = mem1[read_pointer[13:0]];
     end
     else   
     begin
       rdata_upper = mem_alt2[read_pointer[13:0]];
       rdata_lower = mem2[read_pointer[13:0]];
     end
   end
  
       
 // assign rdata_upper = mem_alt[read_pointer];
 // assign rdata_lower = mem[read_pointer];
