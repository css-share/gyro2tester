...
// Instantiation of Axi Bus Interface S00_AXI
  ...
  logic middle_tready;
  logic middle_tvalid;
  logic [31:0] middle_tdata;
  logic s00_axis_tvalid_s;
  ...
  always @ (posedge txclk or negedge tx_rstn)
   ...
   begin
    if (~tx_rstn)
      ...
      s00_axis_tvalid_s <= s00_axis_tvalid;
   end  

  
   pipe_que_2xn #(.WIDTH(32)) u_rx_pipe32 (
                                          .clk(txclk),
                                          .rst_n(tx_rstn),

                                          .vld_in(rx_word_toggle & s00_axis_tvalid_s),
                                          .que_din(rx_double_word),
                                          .rdy_in(s00_axis_tready),
                                                       
                                          .vld_out(middle_tvalid),
                                          .que_dout(middle_tdata),
                                          .rdy_out(middle_tready),    
                                          .que_empty()
                                         );
  ...
  //////////////////////////////////////////////////////////////////
  // Write side logic 
  assign middle_tready = ~(rx_fifo_afull) & ~(rx_fifo_halt);
  assign rx_fifo_push = middle_tvalid & middle_tready;
 ...
 // Read side logic 
  ...
  assign rx_fifo_pop  = m00_axis_tvalid & m00_axis_tready;
    ...
    gen_async_que #(.DPWR(15), .WD(32), .AF(8192), .FIFO_RESET(0))  u_rx_async_stream_fifo (
      .qout       (m00_axis_tdata),
      .pop        (rx_fifo_pop),
      .rd_clk     (clk),
      .rrst_n     (rst_n),
